...
rule withdraw_consistency() {
    ...
    uint256 userBalanceBeforeWithdraw = token.balanceOf(sender);

    token.withdraw(amount);

    assert token.getContractBalance() == contractBalanceBefore - amount;
    assert token.balanceOf(sender) == userBalanceBefore + amount;
}
...
rule withdraw_balance_check() {
    ...
    assert token.getContractBalance() == initialContractBalance - amount;
    assert token.balanceOf(sender) == initialUserBalance - amount;
}