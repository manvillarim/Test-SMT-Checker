...
rule withdraw_consistency() {
    ...
    assert token.getContractBalance() == assert_uint256(contractBalanceBefore - amount);
    assert token.balanceOf(e.sender) == assert_uint256(userBalanceBefore + amount);
}
...
rule withdraw_balance_check() {
    ...
    uint256 initialUserBalance = token.balanceOf(e.sender);
